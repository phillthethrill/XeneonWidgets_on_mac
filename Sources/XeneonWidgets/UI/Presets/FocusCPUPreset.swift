...
struct FocusCPUPreset: View {
    ...
    private func box(for placement: BoxPlacement) -> some View {
        switch placement.id {
        case .cpu:
            CPUBox(cpu: env.cpu, state: state, mode: .focus, uptime: clock.uptime, glow: glow(for: .cpu))
        case .mem:
            MemBox(memory: env.memory, disks: env.disks, compact: true, glow: glow(for: .mem))
        case .net:
            NetBox(network: env.network, compact: true, glow: glow(for: .net))
        case .proc:
